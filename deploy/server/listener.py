...
Dependencies: Python 3.9+ standard library, plus optional ``pynput`` for global
server hotkeys (Ctrl+Alt+T / Ctrl+Alt+Q). Install with ``pip install pynput``.
"""
...
def broadcast_verb(
    ...
    """Send the same verb line to every live session (used by server hotkeys).

    Args:
        registry: Active sessions.
        cfg: Listener configuration (command timeout).
        verb: Full request line, e.g. ``PLAY`` or ``STOP``.
        log: Logger for outcomes.
    """
    sessions = registry.snapshot()
    if not sessions:
        log.info("hotkey broadcast %r: no connected targets", verb)
        return
    for s in sessions:
        reply = send_command(s, verb, cfg.command_timeout)
        log.info("hotkey broadcast %r sid=%d -> %r", verb, s.sid, reply)
...
def cli_loop(
    ...
    Commands:
        ...
        (global hotkeys)         Ctrl+Alt+T = broadcast PLAY; Ctrl+Alt+Q = broadcast STOP
                                 (requires pynput; Linux needs a GUI / X11 session).
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    parser.add_argument(
        "--no-hotkeys",
        action="store_true",
        help="do not register Ctrl+Alt+T / Ctrl+Alt+Q global hotkeys",
    )
    ...
    if not args.no_hotkeys:
        hotkey_thread = threading.Thread(
            target=_global_hotkey_loop,
            args=(registry, cfg, stop_event, log),
            name="hotkeys",
            daemon=True,
        )
        hotkey_thread.start()
...
def _global_hotkey_loop(
    registry: SessionRegistry,
    cfg: SessionConfig,
    stop_event: threading.Event,
    log: logging.Logger,
) -> None:
    """Run ``pynput`` global hotkeys until ``stop_event`` is set.

    Ctrl+Alt+T broadcasts ``PLAY`` (default clip on each target). Ctrl+Alt+Q
    broadcasts ``STOP``. Bare ``q`` is not used so typing in this CLI stays safe.
    """
    try:
        from pynput import keyboard
    except ImportError:
        log.warning(
            "pynput not installed; server hotkeys disabled "
            "(pip install pynput). Ctrl+Alt+T / Ctrl+Alt+Q will do nothing."
        )
        stop_event.wait()
        return

    def on_deploy() -> None:
        broadcast_verb(registry, cfg, "PLAY", log)

    def on_quit_media() -> None:
        broadcast_verb(registry, cfg, "STOP", log)

    hotkeys = keyboard.GlobalHotKeys(
        {
            "<ctrl>+<alt>+t": on_deploy,
            "<ctrl>+<alt>+q": on_quit_media,
        }
    )
    try:
        hotkeys.start()
    ...
    log.info(
        "server hotkeys: Ctrl+Alt+T = broadcast PLAY, Ctrl+Alt+Q = broadcast STOP"
    )
    try:
        stop_event.wait()
    finally:
